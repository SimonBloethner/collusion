...
if local:
    path_ = '/Users/Simon/Documents/Projects/EWF/Research/PhD/ABMs/collusion'
else:
    path_ = 'collusion'
...
class Firm:
    ...
    def update(self):
        ...
        for k in range(self.K_epochs):
            if self.id == 1 and model.period == 302037 and k == 20:
                print('!!')
...
class CollusionModel():
    ...
    def step(self):
        ...
        self.demand[self.period, :] = demand(action, shares[self.period, :], self.num_agents)
        self.profits[self.period, :] = profit(action, shares[self.period, :], self.num_agents)
        ...
        self.period += 1
        if self.period == iterations:
            for a in range(len(self.schedule)):
                self.schedule[a].save(checkpoint_path='{}/agent_{}.pth'.format(checkpoint_path, a))


def demand(p, share, n_firms):
    d = [1 - p[firm] * (1 - share[firm]) + 0.5 * np.max(p[:firm] + p[firm+1:]) for firm in range(len(p))]
    return d


def profit(p, share, n_firms):
    pi = np.array(p) * demand(p, share, n_firms)
    return pi
